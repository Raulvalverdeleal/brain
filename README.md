...
```bash
git clone https://github.com/your-org/agent-skills ~/.skills
```

This is your local registry. All available skills live in `~/.skills/skills/`.
...
```bash
ln -s ~/.skills/scripts/spm.py /usr/local/bin/spm
chmod +x /usr/local/bin/spm
```

**Option B — shell alias**:
```bash
echo 'alias spm="python3 ~/.skills/scripts/spm.py"' >> ~/.zshrc
source ~/.zshrc
```
...
```bash
spm sync    # runs git pull in ~/.skills
```
...
`spm` is the CLI that copies skills into your projects.

```bash
# Download the script
curl -o /usr/local/bin/spm https://raw.githubusercontent.com/your-org/agent-skills/main/scripts/spm.py
...
### 4. Keep the registry up to date
